...
class multilift_model:
    ...
    def minisnap_load_circle(self,Coeffx,Coeffy,Coeffz,time): # 3 trajectories
        t_switch = 0
        t1 = 2.4
        # t2 = t1+1
        if time <t1:
        ...
        delta = 1e-8
        ac    = (ref_vx*ref_ay-ref_vy*ref_ax)/(ref_vx**2+ref_vy**2+delta)*np.reshape(np.vstack((-ref_vy,ref_vx)),(2,1))
