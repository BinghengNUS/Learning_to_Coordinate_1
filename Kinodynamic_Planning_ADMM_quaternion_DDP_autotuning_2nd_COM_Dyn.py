...
class MPC_Planner:
    ...
    def SetStateVariables(self, xl, xi):
        ...
        self.scxL  = SX.sym('scxL',self.nxl) # Lagrangian multiplier of xl for the consensus constraint
        self.scxI  = SX.sym('scxI',self.nxi) # Lagrangian multiplier of xi for the consensus constraint
    ...
    def SetCtrlVariables(self, ul, ui):
        ...
        self.scuL  = SX.sym('scuL',self.nul) # Lagrangian multiplier of ul for the consensus constraint
        self.scuI  = SX.sym('scuI',self.nui) # Lagrangian multiplier of ui for the consensus constraint
    ...
    def ADMM_SubP2_Init(self):
        ...
        self.ubg2  += [1e4]
        for i in range(int(self.nq)):
            scxi_k  = SX.sym('scx'+str(i),self.nxi) # define agent-wise safe copy variables for each cable
            w2     += [scxi_k]
            self.lbw2  += self.scxi_lb
            self.ubw2  += self.scxi_ub
            scxc_k[i*self.nxi:(i+1)*self.nxi] = scxi_k # stack all the safe copy variables for each cable into a single vector
            ...
            J2     += self.Ji_P2_k_fn(xi0=xi_k,scxi0=scxi_k,scxI0=scxI_k,ui0=ui_k,scui0=scui_k,scuI0=scuI_k, parai0=para_i, a0=a)['Ji_P2_kf'] # agent-wise cost function
        
        for i in range(int(self.nq)):    
            # safe constriant between the obstacle 1 and the ith quadrotor
            goi1       = self.Gi1[i](scxl0=scxl_k,scxc0=scxc_k)['go1f'+str(i)] # defined using stacked safe-copy variables across all agents at time-step k
            g2        += [goi1]
            self.lbg2 += [1e-2]
    ...
    def SubP2_Gradient(self,auxSys2,grad_outl,grad_outc,scxL_grad,scuL_grad,scxC_grad,scuC_grad,px,pu,gammax,gammau,pix,piu,gammaix,gammaiu,ADMM_max,i_admm):
        ...
        for k in range(self.N):
            ...
            gradtimeSubP2_k    = (TM.time() - start_time)*1000
            Grad2Time       += gradtimeSubP2_k
        ...
        print('ADMM iteration:',i_admm+1,"Eig_time:--- %s ms ---" % format(EigTime,'.2f')) 
        print('ADMM iteration:',i_admm+1,"GSubP2_time:--- %s ms ---" % format(Grad2Time,'.2f'))  
        grad_out2 = {
                    "scxl_grad":scxl_grad,
                    "scul_grad":scul_grad,
                    "scxc_grad":scxc_grad,
    ...
    def Get_AuxSys_Centralized(self,Opt_Sol1_l,Opt_Sol1_cddp,auxSysl,AuxSysc,auxSys2,px,pu,gammax,gammau,pix,piu,gammaix,gammaiu,ADMM_max,i_admm):
        px_dis     = self.open_loop_penalty(px,gammax,i_admm,ADMM_max)
        pu_dis     = self.open_loop_penalty(pu,gammau,i_admm,ADMM_max)
        pix_dis    = self.open_loop_penalty(pix,gammaix,i_admm,ADMM_max)
        piu_dis    = self.open_loop_penalty(piu,gammaiu,i_admm,ADMM_max)
        Ixl        = np.identity(self.nxl)
        Iul        = np.identity(self.nul)
        Ixi        = np.identity(self.nxi)
        Iui        = np.identity(self.nui)
        Ix         = np.identity(self.nxl+int(self.nq)*self.nxi)
        Iu         = np.identity(self.nul+int(self.nq)*self.nui)
        # load the trajectories from the forward pass
        Hxlxl      = Opt_Sol1_l['Hxx']
        Hxlul      = Opt_Sol1_l['Hxu']
        Hulul      = Opt_Sol1_l['Huu']
        Fl         = Opt_Sol1_l['Fx']
        Gl         = Opt_Sol1_l['Fu']
        HxlxlN     = auxSysl['HxxN']
        HxlNp      = auxSysl['HxNp']
        ...
        Hucp       = []
        for i in range(int(self.nq)):
            Hxcxc  += [Opt_Sol1_cddp[i]['Hxx']]
            Hxcuc  += [Opt_Sol1_cddp[i]['Hxu']]
            Hucuc  += [Opt_Sol1_cddp[i]['Huu']]
            Fc     += [Opt_Sol1_cddp[i]['Fx']]
            Gc     += [Opt_Sol1_cddp[i]['Fu']]
            HxcxcN += [AuxSysc[i]['HxxN']]
            HxcNp  += [AuxSysc[i]['HxNp']]
            Hxcp   += [AuxSysc[i]['Hxp']]
            Hucp   += [AuxSysc[i]['Hup']]
        Lscxlscxl_o  = auxSys2['Lscxlscxl_o']
        Lscxlscul_o  = auxSys2['Lscxlscul_o']
        ...
        Lscucp       = auxSys2['Lscucp']
        # subproblem1
        Hxx        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nxl+int(self.nq)*self.nxi)) for _ in range(self.N+1)] 
        Hxu        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nul+int(self.nq)*self.nui)) for _ in range(self.N)] 
        Huu        = [np.zeros((self.nul+int(self.nq)*self.nui, self.nul+int(self.nq)*self.nui)) for _ in range(self.N)] 
        Hxp        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.n_Pauto)) for _ in range(self.N+1)]
        Hup        = [np.zeros((self.nul+int(self.nq)*self.nui, self.n_Pauto)) for _ in range(self.N)]
        F          = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nxl+int(self.nq)*self.nxi)) for _ in range(self.N)]
        G          = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nul+int(self.nq)*self.nui)) for _ in range(self.N)]
        # subproblem2
        Lxx        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nxl+int(self.nq)*self.nxi)) for _ in range(self.N+1)]
        Lxu        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nul+int(self.nq)*self.nui)) for _ in range(self.N)]
        Luu        = [np.zeros((self.nul+int(self.nq)*self.nui, self.nul+int(self.nq)*self.nui)) for _ in range(self.N)]
        Lxp        = [np.zeros((self.nxl+int(self.nq)*self.nxi, self.n_Pauto)) for _ in range(self.N+1)]
        Lup        = [np.zeros((self.nul+int(self.nq)*self.nui, self.n_Pauto)) for _ in range(self.N)]

        for k in range(self.N):
            # subproblem 1, concatenate matrices
            Hxx[k][0:self.nxl,0:self.nxl] = Hxlxl[k] - px_dis*Ixl
            Hxu[k][0:self.nxl,0:self.nul] = Hxlul[k] 
            Huu[k][0:self.nul,0:self.nul] = Hulul[k] - pu_dis*Iul
            Hxp[k][0:self.nxl,0:self.n_Pauto] = Hxlp[k]
            Hup[k][0:self.nul,0:self.n_Pauto] = Hulp[k]
            F[k][0:self.nxl,0:self.nxl]   = Fl[k]
            G[k][0:self.nxl,0:self.nul]   = Gl[k]
            for i in range(int(self.nq)):
                Hxx[k][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi] = Hxcxc[i][k] - pix_dis*Ixi
                Hxu[k][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,self.nul+i*self.nui:self.nul+(i+1)*self.nui] = Hxcuc[i][k]
                Huu[k][self.nul+i*self.nui:self.nul+(i+1)*self.nui,self.nul+i*self.nui:self.nul+(i+1)*self.nui] = Hucuc[i][k] - piu_dis*Iui
                Hxp[k][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,0:self.n_Pauto] = Hxcp[i][k]
                Hup[k][self.nul+i*self.nui:self.nul+(i+1)*self.nui,0:self.n_Pauto] = Hucp[i][k]
                F[k][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi] = Fc[i][k]
                G[k][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,self.nul+i*self.nui:self.nul+(i+1)*self.nui] = Gc[i][k]
            # subproblem 2, concatenate matrices
            Lxx_k  = np.block([[Lscxlscxl_o[k],  Lscxlscxc_o[k]],
                               [Lscxlscxc_o[k].T,Lscxcscxc_o[k]]])
            Lxu[k] = np.block([[Lscxlscul_o[k],  Lscxlscuc_o[k]],
                               [Lsculscxc_o[k].T,Lscxcscuc_o[k]]])
            Luu_k = np.block([[Lsculscul_o[k],  Lsculscuc_o[k]],
                               [Lsculscuc_o[k].T,Lscucscuc_o[k]]])
            L_hessian_ko = np.block([[Lxx_k,  Lxu[k]],
                                    [Lxu[k].T,Luu_k]])
            min_eigval = np.min(LA.eigvalsh(L_hessian_ko))
            if min_eigval<0:
                reg = -min_eigval+1e-4
            ...
            Lxx[k] = Lxx_k + reg*Ix
            Luu[k] = Luu_k + reg*Iu
            Lxp[k] = np.block([[Lscxlp[k]],
                               [Lscxcp[k]]])
            Lup[k] = np.block([[Lsculp[k]],
                               [Lscucp[k]]])
        # subproblem 1, terminal concatenated matrices
        Hxx[self.N][0:self.nxl,0:self.nxl] = HxlxlN - px_dis*Ixl
        Hxp[self.N][0:self.nxl,0:self.n_Pauto] = HxlNp
        for i in range(int(self.nq)):
            Hxx[self.N][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi] = HxcxcN[i] - pix_dis*Ixi
            Hxp[self.N][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi,0:self.n_Pauto] = HxcNp[i]
        # subproblem 2, terminal concatenated matrices
        Lxx_N = np.block([[Lscxlscxl_o[self.N],  Lscxlscxc_o[self.N]],
                          [Lscxlscxc_o[self.N].T,Lscxcscxc_o[self.N]]])
        min_eigval = np.min(LA.eigvalsh(Lxx_N))
        if min_eigval<0:
            reg = -min_eigval+1e-4
        else:
            reg = 0
        Lxx[self.N] = Lxx_N + reg*Ix
        Lxp[self.N] = np.block([[Lscxlp[self.N]],
                                [Lscxcp[self.N]]])
        print(Hxx[0] is Hxx[1])
        print(Hxu[0] is Hxu[1])
        print(F[0] is F[1])
        print(Lxx[0] is Lxx[1])
        
        auxSys_Central = {"Hxx":Hxx,
                          "Hxu":Hxu,
                          "Huu":Huu,
                          "Hxp":Hxp,
                          "Hup":Hup,
        ...
        return auxSys_Central



    def Centralized_Gradient(self,auxSys_Central):
        Hxx, Hxu, Huu = auxSys_Central['Hxx'], auxSys_Central['Hxu'], auxSys_Central['Huu']
        Hxp, Hup, Lxx = auxSys_Central['Hxp'], auxSys_Central['Hup'], auxSys_Central['Lxx']
        Lxu, Luu, Lxp = auxSys_Central['Lxu'], auxSys_Central['Luu'], auxSys_Central['Lxp']
        Lup, F,   G   = auxSys_Central['Lup'], auxSys_Central['F'], auxSys_Central['G']
        X_grad        = (self.N+1) * [np.zeros((self.nxl+int(self.nq)*self.nxi, self.n_Pauto))]
        U_grad        = self.N * [np.zeros((self.nul+int(self.nq)*self.nui, self.n_Pauto))]
        Kfb           = self.N * [np.zeros((self.nul+int(self.nq)*self.nui, self.nxl+int(self.nq)*self.nxi))]
        Kff           = self.N * [np.zeros((self.nul+int(self.nq)*self.nui, self.n_Pauto))]
        Vxx           = (self.N+1) * [np.zeros((self.nxl+int(self.nq)*self.nxi, self.nxl+int(self.nq)*self.nxi))]
        Vxp           = (self.N+1) * [np.zeros((self.nxl+int(self.nq)*self.nxi, self.n_Pauto))]
        Vxx[self.N]   = Hxx[self.N] + Lxx[self.N]
        Vxp[self.N]   = Hxp[self.N] + Lxp[self.N]
        #-------Backward recursion-------#    
        for k in reversed(range(self.N)): # N-1, N-2, ....., 0
            Qxx_k     = Hxx[k] + Lxx[k] + F[k].T@Vxx[k+1]@F[k]
            Qux_k     = Hxu[k].T + Lxu[k].T + G[k].T@Vxx[k+1]@F[k]
            Quu_k     = Huu[k] + Luu[k] + G[k].T@Vxx[k+1]@G[k]
            Qxp_k     = Hxp[k] + Lxp[k] + F[k].T@Vxp[k+1]
            Qup_k     = Hup[k] + Lup[k] + G[k].T@Vxp[k+1]
            invQuu_k  = LA.inv(Quu_k)
            Kfb[k]    = -invQuu_k@Qux_k
            Kff[k]    = -invQuu_k@Qup_k
            Vxx[k]    = Qxx_k + Qux_k.T@Kfb[k]
            Vxp[k]    = Qxp_k + Qux_k.T@Kff[k]
        #-------Foreward recursion-------#
        for k in range(self.N):
            U_grad[k]   = Kfb[k]@X_grad[k]+Kff[k]
            X_grad[k+1] = F[k]@X_grad[k] + G[k]@U_grad[k]
        
        Grad_out = {"Xc_grad": X_grad,
                    "Uc_grad": U_grad}
    ...
    def ADMM_Gradient_Solver(self,Opt_Sol1_l, Opt_Sol1_cddp, Opt_Sol1_c, Opt_Sol2, Opt_Sol3, Ref_xl, Ref_ul, ref_xq, ref_uq, weight1, weight2):
        ...
        for i_admm in range(self.max_iter_ADMM):
            ...
            grad_out2, Grad2Time  = self.SubP2_Gradient(auxSys2,grad_outl,grad_outc,scxL_grad,scuL_grad,scxC_grad,scuC_grad,weight1[-4],weight1[-3],weight1[-2],weight1[-1],weight2[-4],weight2[-3],weight2[-2],weight2[-1],int(self.max_iter_ADMM),i_admm) 
            ...
            gradtime_3 = (TM.time() - start_time)*1000
        ...
        # # centralized method
        # auxSys_Central = self.Get_AuxSys_Centralized(Opt_Sol1_l[i_admm],Opt_Sol1_cddp[i_admm],auxSysl,AuxSysc,auxSys2,weight1[-4],weight1[-3],weight1[-2],weight1[-1],weight2[-4],weight2[-3],weight2[-2],weight2[-1],int(self.max_iter_ADMM),i_admm)
        # start_time     = TM.time()
        # Grad_out       = self.Centralized_Gradient(auxSys_Central)
        # gradtime_cent  = (TM.time() - start_time)*1000
        # X_grad         = Grad_out['Xc_grad']
        # U_grad         = Grad_out['Uc_grad']
        # print("gtime_centralized:--- %s ms ---" % format(gradtime_cent,'.2f'))
        # gcError1     = 0
        # gcErroru1    = 0
        # gcError1_c   = 0
        # gcErroru1_c  = 0
        # for k in range(self.N):
        #     gcerror1   = X_grad[k+1][0:self.nxl,0:self.n_Pauto] - xl_grad[k+1]
        #     gcError1  += LA.norm(gcerror1,ord='fro')/LA.norm(xl_grad[k+1],ord='fro')
        #     gcerroru1  = U_grad[k][0:self.nul,0:self.n_Pauto] - ul_grad[k]
        #     gcErroru1 += LA.norm(gcerroru1,ord='fro')/LA.norm(ul_grad[k],ord='fro')
        #     for i in range(int(self.nq)):
        #         gcerror1_c   = X_grad[k+1][self.nxl+i*self.nxi:self.nxl+(i+1)*self.nxi, 0:self.n_Pauto] - grad_outc[i]['xi_grad'][k+1]
        #         gcError1_c  += LA.norm(gcerror1_c,ord='fro')/LA.norm(grad_outc[i]['xi_grad'][k+1],ord='fro')
        #         gcerroru1_c  = U_grad[k][self.nul+i*self.nui:self.nul+(i+1)*self.nui, 0:self.n_Pauto] - grad_outc[i]['ui_grad'][k] 
        #         gcErroru1_c += LA.norm(gcerroru1_c,ord='fro')/LA.norm(grad_outc[i]['ui_grad'][k],ord='fro')
        # gmeancerror1 = np.sqrt(gcError1**2+gcErroru1**2)/self.N
        # gmeancerror1_c = np.sqrt(gcError1_c**2+gcErroru1_c**2)/(self.N*self.nq) 
        # print('gmeanerror1_Dis_vs_Cent=',gmeancerror1,'gmeanerror1_c_Dis_vs_Cent=',gmeancerror1_c)
        
        return Grad_Out1l, Grad_Out1c, Grad_Out2, Grad_Out3, GradTime, GradTimeCaos, GradTimeCao,  GradTimePDP,  GradTime_c, GradTimeCaos_c, GradTimeCao_c, GradTimePDP_c,  MeanerrorCao, MeanerrorPDP, MeanerrorCao_c, MeanerrorPDP_c, gMeanerror_l, gMeanerror_c, backward_time
...
class Gradient_Solver:
    ...
    def adaptive_meta_loss_weights(self, loss_t, loss_rp, g_t, g_rp, wt, alpha=7, beta_w=0.8, eps=1e-8, k_min=0.2, k_max=200.0):
        ...
        loss_t = float(np.asarray(loss_t, dtype=np.float64).reshape(-1)[0])
        loss_rp = float(np.asarray(loss_rp, dtype=np.float64).reshape(-1)[0])
        g_t = float(np.asarray(g_t, dtype=np.float64).reshape(-1)[0])
        g_rp = float(np.asarray(g_rp, dtype=np.float64).reshape(-1)[0])
        wt = float(np.asarray(wt, dtype=np.float64).reshape(-1)[0])
