...
class MPC_Planner:
    ...
    def SetConstraints_ADMM_Subp2(self, pob1, pob2):
        ...
        for i in range(int(self.nq)):
            ...
            self.t_min = 0.1
            self.t_max = 20
            self.T_cons += -self.p_bar * log(ti_m-self.t_min)
            self.T_cons += -self.p_bar * log(self.t_max-ti_m)
            self.Ti_admm += [ti_fn]
        k = 0
    ...
    def DDP_ADMM_Subp1(self,xl_0,Ref_xl,Ref_Wl,weight1,scxl_opt_hat,scWl_opt_hat,Y_l,Eta_l,p1,max_iter,e_tol):
        ...
        while ratio>e_tol and iteration<=max_iter:
            ...
            for k in reversed(range(self.N)): # N-1, N-2,...,0
                ...
                Quu_k     = Quu_bar_k + Quu_hat_k #+ reg*np.identity(self.n_Wl) # regularized
    ...
    def SubP2_Gradient(self,auxSys2,grad_out,scxL_grad,scWL_grad,p1):
        ...
        for k in range(self.N):
            ...
            min_eigval = np.min(LA.eigvalsh(L_hessian_k))
            max_eigval = np.max(LA.eigvalsh(L_hessian_k))
            # print('min_eigval=',min_eigval,'max_eigval=',max_eigval)
            if min_eigval<0:
                reg = -min_eigval+1e-8
            else:
                reg = 0
            L_hessian_k_inv = LA.inv(L_hessian_k+ reg*np.identity(self.n_xl+self.n_Wl+self.n_nv)) #
...
class Gradient_Solver:
    ...
    def ChainRule_Gradient(self,tunable_para):
        ...
        for k in range(self.n_P):
            ...
            # Weight[k]= self.p_min + Tunable[k]**2
