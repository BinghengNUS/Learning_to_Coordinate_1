...
import Kinodynamic_Planning_ADMM_quaternion_DDP_autotuning_2nd_COM_Dyn_true_parallel_best_backup
import math
import time as TM
...
def _max_abs_diff(a, b):
    if isinstance(a, dict):
        return max((_max_abs_diff(a[k], b[k]) for k in a.keys()), default=0.0)
    ...
    return float(np.max(np.abs(a_np - b_np))) if a_np.size > 0 else 0.0


COMPARE_LA_REFERENCE = os.environ.get("COMPARE_LA_REFERENCE", "0") == "1"
COMPARE_ITER = int(os.environ.get("COMPARE_ITER", "1"))
COMPARE_TASK = int(os.environ.get("COMPARE_TASK", "0"))
...
mq        = 0.25  # the quadrotor mass [kg] *0.7 for nq=8, 0.5 for nq=12
fqmax     = 0.75*9.81 # the quadrotor maximum thrust [N]
cl0       = 1     # the cable length [m]
rq        = 0.15  # the radius of quadrotor [m], *0.7 for nq=8, 0.5 for nq=12
...
MPC_load  = Kinodynamic_Planning_ADMM_quaternion_DDP_autotuning_2nd_COM_Dyn_true_parallel_best_backup.MPC_Planner(sysm_para,dt,horizon)
...
MPC_load.enable_cable_parallel_diagnostics = True
...
# parameters of ADAM
lr0       = 0.2 # 0.2 for training, 1e-3 for testing gradient time
lr_nn     = lr0 
epsilon   = 1e-8
...
Grad_Solver   = Kinodynamic_Planning_ADMM_quaternion_DDP_autotuning_2nd_COM_Dyn_true_parallel_best_backup.Gradient_Solver(sysm_para, horizon, MPC_load.xl, MPC_load.ul, MPC_load.scxl, MPC_load.scul, MPC_load.xi, MPC_load.ui, MPC_load.scxi, MPC_load.scui, MPC_load.P_auto, MPC_load.para_l, MPC_load.para_i)

if COMPARE_LA_REFERENCE:
    MPC_load_ref = Kinodynamic_Planning_ADMM_quaternion_DDP_autotuning_2nd_COM_Dyn.MPC_Planner(sysm_para, dt, horizon)
    MPC_load_ref.Rotational_Inertia(rp0)
    MPC_load_ref.allocation_martrix(rg0)
    MPC_load_ref.SetStateVariables(sysm.xl, sysm.xi)
    MPC_load_ref.SetCtrlVariables(sysm.ul, sysm.ui)
    MPC_load_ref.SetDyns(sysm.model_l, sysm.model_i)
    MPC_load_ref.SetWeightPara()
    MPC_load_ref.SetPayloadCostDyn(max_iter_ADMM)
    MPC_load_ref.SetCableCostDyn(max_iter_ADMM)
    MPC_load_ref.SetConstriants(pob1, pob2)
    MPC_load_ref.SetADMMSubP2_SoftCost_k()
    MPC_load_ref.SetADMMSubP2_SoftCost_N()
    MPC_load_ref.ADMM_SubP2_Init()
    MPC_load_ref.ADMM_SubP2_N_Init()
    MPC_load_ref.Load_derivatives_DDP_ADMM()
    MPC_load_ref.Cable_derivatives_DDP_ADMM()
    MPC_load_ref.system_derivatives_SubP2_ADMM_k()
    MPC_load_ref.system_derivatives_SubP2_ADMM_N()
    MPC_load_ref.system_derivatives_SubP3_ADMM()
    MPC_load_ref.enable_cable_parallel_diagnostics = False
    Grad_Solver_ref = Kinodynamic_Planning_ADMM_quaternion_DDP_autotuning_2nd_COM_Dyn.Gradient_Solver(
        sysm_para,
        horizon,
        MPC_load_ref.xl,
        MPC_load_ref.ul,
...
PATHl_init    = "trained_data_multiagent_meta_COM_Dyn/initial_NN_l.pt"
PATHi_init    = "trained_data_multiagent_meta_COM_Dyn/initial_NN_i.pt"
# PATHl_init    = "trained_data_multiagent_meta_COM_Dyn/trained_nn_l_4_3_n.pt"
# PATHi_init    = "trained_data_multiagent_meta_COM_Dyn/trained_nn_i_4_3_n.pt"
...
def train(m0,v0,lr0,lr_nn,Tunable_para0,NN_l,NN_i,wt0,wrp0,max_iter_ADMM,initial_model):
    ...
    # for i_iter in range(1,19):
        ...
        for task_idx in range(num_task):
            ...
            MPC_load.system_derivatives_SubP2_ADMM_N()
            if COMPARE_LA_REFERENCE:
                MPC_load_ref.Rotational_Inertia(rp_task[task_idx])
                MPC_load_ref.allocation_martrix(rg_task)
                MPC_load_ref.SetDyns(sysm.model_l, sysm.model_i)
                MPC_load_ref.SetConstriants(pob1, pob2)
                MPC_load_ref.SetADMMSubP2_SoftCost_k()
                MPC_load_ref.SetADMMSubP2_SoftCost_N()
            ...
            xl_init_task[0] = xl_init[0] + float(np.squeeze(rg_task[0]))
            xl_init_task[1] = xl_init[1] + float(np.squeeze(rg_task[1]))
            ...
            # start_time = TM.time()
            MPC_load.debug_train_iter = i_iter
            MPC_load.debug_task_idx = task_idx
            output = MPC_load.ADMM_Gradient_Solver(Opt_Sol1_l, Opt_Sol1_cddp, Opt_Sol1_c, Opt_Sol2, Opt_Sol3, Ref_xl, Ref_ul, ref_xc, ref_uq, P_weight1, P_weight2)
            # gradtime    = (TM.time() - start_time)*1000
            print("backward:--- %s ms ---" % format(output["backward_time"],'.2f'))
            gradtimeOur    += [output["GradTime"][-1]]
            gradtimeCaos   += [output["GradTimeCaos"][-1]]
            gradtimeCao    += [output["GradTimeCao"][-1]]
            gradtimePDP    += [output["GradTimePDP"][-1]]
            gradtimeOur_c  += [output["GradTime_c"][-1]]
            gradtimeCaos_c += [output["GradTimeCaos_c"][-1]]
            gradtimeCao_c  += [output["GradTimeCao_c"][-1]]
            gradtimePDP_c  += [output["GradTimePDP_c"][-1]]
            # gradtime_Central+= [gradtime_cent]
            meanerrorCao   += [output["MeanerrorCao"][-1]]
            meanerrorPDP   += [output["MeanerrorPDP"][-1]]
            meanerrorCao_c += [output["MeanerrorCao_c"][-1]]
            meanerrorPDP_c += [output["MeanerrorPDP_c"][-1]]
            gMeanerror_load += [output["gMeanerror_l"]]
            gMeanerror_cable += [output["gMeanerror_c"]]
            if i_iter > 1:
                gradtime_1_avg += [output["Gradtime_1_avg"]]
                gradtime_2_avg += [output["Gradtime_2_avg"]]
                gradtime_3_avg += [output["Gradtime_3_avg"]]
                np.save('trained_data_multiagent_meta_COM_Dyn/gradtime_1_avg_'+str(horizon)+'_'+str(nq),gradtime_1_avg)
                np.save('trained_data_multiagent_meta_COM_Dyn/gradtime_2_avg_'+str(horizon)+'_'+str(nq),gradtime_2_avg)
                np.save('trained_data_multiagent_meta_COM_Dyn/gradtime_3_avg_'+str(horizon)+'_'+str(nq),gradtime_3_avg)
                ...
                # np.save('trained_data_multiagent_meta_COM_Dyn/xl_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),xl_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/ul_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),ul_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/Kfbl_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),Kfbl_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/scxl_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),scxl_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/scul_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),scul_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/xc_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),xc_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/uc_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),uc_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/scxc_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),scxc_task)
                # np.save('trained_data_multiagent_meta_COM_Dyn/scuc_train_'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode)+'_'+str(nq),scuc_task)
            Grad_Out1l      = output["Grad_Out1l"]
            Grad_Out1c      = output["Grad_Out1c"]
            Grad_Out2       = output["Grad_Out2"]
            ...
            if COMPARE_LA_REFERENCE and i_iter == COMPARE_ITER and task_idx == COMPARE_TASK:
                output_ref = MPC_load_ref.ADMM_Gradient_Solver(
                    Opt_Sol1_l,
                    Opt_Sol1_cddp,
                    Opt_Sol1_c,
                    Opt_Sol2,
                ...
                dldw_ref, loss_ref, loss_track_ref, loss_resid_ref, gloss_t_ref, gloss_rp_ref = Grad_Solver_ref.ChainRule(
                    Opt_Sol1_l,
                    Opt_Sol1_c,
                    Opt_Sol2,
                    Ref_xl,
                    ref_xc,
                    output_ref["Grad_Out1l"],
                    output_ref["Grad_Out1c"],
                    output_ref["Grad_Out2"],
                    wt,
                    wrp,
                )
                print(
                    ...
                    f"iter_train={i_iter}",
                    f"task_idx={task_idx}",
                    f"Grad_Out1l_max={_max_abs_diff(Grad_Out1l, output_ref['Grad_Out1l']):.6e}",
                    f"Grad_Out1c_max={_max_abs_diff(Grad_Out1c, output_ref['Grad_Out1c']):.6e}",
                    f"Grad_Out2_max={_max_abs_diff(Grad_Out2, output_ref['Grad_Out2']):.6e}",
                    f"dldw_max={_max_abs_diff(dldw, dldw_ref):.6e}",
                    f"loss_diff={_max_abs_diff(loss, loss_ref):.6e}",
                    f"loss_t_diff={_max_abs_diff(loss_track, loss_track_ref):.6e}",
                    f"loss_rp_diff={_max_abs_diff(loss_resid, loss_resid_ref):.6e}",
                    f"gloss_t_diff={_max_abs_diff(gloss_t, gloss_t_ref):.6e}",
                    f"gloss_rp_diff={_max_abs_diff(gloss_rp, gloss_rp_ref):.6e}",
                )
    ...
    np.save('trained_data_multiagent_meta_COM_Dyn/xl_train_'+str(horizon)+'_'+str(nq),xl_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/ul_train_'+str(horizon)+'_'+str(nq),ul_task)
    # np.save('trained_data_multiagent_meta_COM_Dyn/Kfbl_train_'+str(horizon)+'_'+str(nq),Kfbl_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/scxl_train_'+str(horizon)+'_'+str(nq),scxl_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/scul_train_'+str(horizon)+'_'+str(nq),scul_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/xc_train_'+str(horizon)+'_'+str(nq),xc_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/uc_train_'+str(horizon)+'_'+str(nq),uc_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/scxc_train_'+str(horizon)+'_'+str(nq),scxc_task)
    np.save('trained_data_multiagent_meta_COM_Dyn/scuc_train_'+str(horizon)+'_'+str(nq),scuc_task)
    ...
    plt.ylabel('Loss')
    plt.grid()
    plt.savefig('trained_data_multiagent_meta_COM_Dyn/loss_train_'+str(horizon)+'_'+str(nq)+'.png',dpi=300)
    plt.show()
...
def evaluate(i_train,task_idx,max_iter_ADMM,initial_model):
    ...
    for k in range(horizon):
        ...
        rl_k            = Rot.from_matrix(np.asarray(Rl_k, dtype=float))
