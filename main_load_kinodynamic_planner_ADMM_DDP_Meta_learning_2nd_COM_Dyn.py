...
def train(m0,v0,lr0,lr_nn,Tunable_para0,NN_l,NN_i,wt0,wrp0,max_iter_ADMM,initial_model):
    ...
    while delta_loss>epi and i_iter<=i_max:
        ...
        for task_idx in range(num_task):
            ...
            rg_task = np.asarray(m2/mtot*rp_task[task_idx], dtype=np.float64).reshape(-1)
    ...
    np.save('trained_data_multiagent_meta_COM_Dyn/gradtime_central'+str(initial_model)+'_'+str(max_iter_ADMM)+'_'+str(weight_mode),gradtime_Central)
...
def evaluate(i_train,task_idx,max_iter_ADMM,initial_model):
    ...
    rg_task = np.asarray(m2/mtot*rp_task[task_idx], dtype=np.float64).reshape(-1) # [m]
    MPC_load.allocation_martrix(rg_task)
    weight_mode1 = 'n'
